...
if [ -f "requirements.txt" ]; then
    ...
    echo -e "${YELLOW}ℹ  Remember to activate venv before running scripts:${NC}"
...
# Activate venv for testing if it exists
if [ -d "venv" ]; then
    source venv/bin/activate
fi
...
echo "Python environment: $([ -d venv ] && echo "Virtual environment (venv)" || echo "System Python")"
echo ""
echo "Next steps:"
echo "  1. cd ~/block-coding-robot"
echo "  2. source venv/bin/activate     # Activate virtual environment"
echo "  3. python3 prepare_datasets.py"
echo "  4. python3 train.py --experiment 2"
echo ""
echo "To access from Windows Explorer:"
echo "  \\\\wsl\$\\Ubuntu\\home\\$(whoami)\\block-coding-robot"
echo ""
echo -e "${YELLOW}Remember: Always activate venv before running Python scripts!${NC}"
echo "  source venv/bin/activate"
echo ""
