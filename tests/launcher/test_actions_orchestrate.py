...
def test_run_setup_happy_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(la.doctor, "internet_reachable", lambda *a, **k: True)
...
def test_run_setup_aborts_offline_before_pip(tmp_path, monkeypatch):
    monkeypatch.setattr(la, "find_system_python", lambda: "python3")
    monkeypatch.setattr(la, "create_venv", lambda *a, **k: True)
    monkeypatch.setattr(la.doctor, "internet_reachable", lambda *a, **k: False)

    def no_pip(*a, **k):
        raise AssertionError("pip_install must not run when offline")
    monkeypatch.setattr(la, "pip_install", no_pip)

    # Generic offline message when the arm isn't reachable either.
    monkeypatch.setattr(la.doctor, "check_arm_reachable",
                        lambda *a, **k: la.doctor.CheckResult("fail", "Arm", "no"))
    logs = []
    ok = la.run_setup(tmp_path, log=logs.append)
    assert ok is False
    assert any("No internet" in ln for ln in logs)
    assert not any("robot's WiFi" in ln for ln in logs)


def test_run_setup_offline_names_robot_wifi_when_arm_reachable(tmp_path, monkeypatch):
    monkeypatch.setattr(la, "find_system_python", lambda: "python3")
    monkeypatch.setattr(la, "create_venv", lambda *a, **k: True)
    monkeypatch.setattr(la.doctor, "internet_reachable", lambda *a, **k: False)
    monkeypatch.setattr(la, "pip_install",
                        lambda *a, **k: (_ for _ in ()).throw(AssertionError("no pip")))
    monkeypatch.setattr(la.doctor, "check_arm_reachable",
                        lambda *a, **k: la.doctor.CheckResult("ok", "Arm", "reachable"))
    logs = []
    ok = la.run_setup(tmp_path, log=logs.append)
    assert ok is False
    assert any("robot's WiFi" in ln for ln in logs)
